...
class Summaries:
    ...
    def fill_peptide_with_sum(self) -> list:
        ...
        return self.cursor.execute(
            ...
            SELECT
                ...
                COUNT(*) AS count,
                SUM(LENGTH(peptide.sequence)) as seq_length_sum,
                SUM(score) AS sc_sum,
                ...
                SUM(score)
                    ...
                    / (SELECT sc_norm_sum FROM norm_sums_per_table
                        WHERE norm_sums_per_table.table_number
                              = peptide.table_number)
                    AS sc_norm_to_file_norm_ratio,
                SUM(peptide_intensity)
                    / LENGTH(sequence.sequence)
                    / (
                        SELECT peptide_intensity_norm_sum
                        FROM norm_sums_per_table
                        WHERE norm_sums_per_table.table_number
                              = peptide.table_number)
