...
class OutputGrouping:
    ...
    def apply_group_filter(
        self,
        max_group_lack: int,
        min_groups_with_accession: int,
    ) -> None:
        """Применение фильтра по группам к peptide_with_sum

        Таблицы, начинающиеся с одинакового числа входят в одну группу,
        например:
        ...
        группе, если он отсутствует в больше, чем maxGroupAbsence таблицах
        внутри группы.

        Args:
            max_group_lack: максимально возможное количество таблиц без
                accession в группе.
            min_groups_with_accession: минимальное количество групп с
                accession.
        """
        ...
        self.cursor.execute(
            ...
            WITH accession_bunch AS (
                ...
                FROM (
                    SELECT DISTINCT
                        GET_GROUP_NUMBER(table_number) AS group_number
            ...
            [max_group_lack, min_groups_with_accession],
        )
